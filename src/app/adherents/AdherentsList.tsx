    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-white rounded-xl p-4 flex flex-col gap-3"
        ...
        <div className="flex gap-3 flex-wrap">
          <input type="text" placeholder="Rechercher nom, prénom, licence…"
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <span className="text-[12px] text-slate-400">
            ...
            {selected.size > 0 && <span className="ml-2 font-medium" style={{ color: 'var(--csn-navy)' }}>— {selected.size} sélectionné{selected.size > 1 ? 's' : ''}</span>}
          </span>
          <div className="flex gap-2">
            <button onClick={() => setSelected(new Set(adherents.filter(a => !a.imprime).map(a => a.id)))}
            ...
            {selected.size > 0 && (
              <button className="text-[11px] px-3 py-1 rounded text-white hover:opacity-90 transition-opacity"
                style={{ background: 'var(--csn-navy)' }}
                onClick={() => alert(`Impression de ${selected.size} étiquette(s) — WebUSB à venir`)}>
          ...
          <table className="w-full" style={{ borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '0.5px solid var(--csn-border-strong)', background: 'var(--csn-cream)' }}>
                <th className="w-8 px-3 py-2.5">
                  <input type="checkbox" checked={selected.size === adherents.length && adherents.length > 0} onChange={toggleAll} className="cursor-pointer" />
                </th>
                <th className="w-8 px-2 py-2.5 text-[10px] text-slate-400 font-medium text-center">Imp.</th>
                ...
                <th className="px-3 py-2.5 text-left text-[11px] text-slate-500 font-medium hidden md:table-cell">Licence</th>
                <th className="px-3 py-2.5 text-left text-[11px] text-slate-500 font-medium hidden lg:table-cell">Expir. licence</th>
            ...
            <tbody>
              {adherents.length === 0 && !isPending && (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-[13px] text-slate-400">Aucun adhérent trouvé</td></tr>
              )}
              {adherents.map((a, i) => {
                const jours = a.caci ? Math.floor((new Date(+a.caci.split('/')[2], +a.caci.split('/')[1] - 1, +a.caci.split('/')[0]).getTime() - Date.now()) / 86400000) : null
                return (
                  <tr key={a.id}
                    ...
                    <td className="px-2 py-2.5 text-center">
                      <span className="inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px]"
                        style={{ background: a.imprime ? '#eaf7f0' : 'var(--csn-cream)', color: a.imprime ? '#1a6642' : '#94a3b8', border: `0.5px solid ${a.imprime ? '#7dd4a8' : 'var(--csn-border)'}` }}>
                        {a.imprime ? '✓' : '○'}
                      </span>
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-[12px] text-slate-400 hidden md:table-cell">{a.licence ?? '—'}</td>
                    <td className="px-3 py-2.5 text-[12px] text-slate-400 hidden lg:table-cell">{a.dateExpiration ?? '—'}</td>
                    <td className="px-3 py-2.5 text-[12px] font-medium" style={{ color: caciColor(a.caci) }}>
